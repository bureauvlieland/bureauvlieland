...
export const useCustomerProgram = (token: string): UseCustomerProgramReturn => {
    ...
    try {
      ...
      if (accomData) {
        const transformedAccom: AccommodationRequest = {
          ...
          board_preference: (accomData as any).board_preference ?? null,
        ...
        const transformedQuotes: AccommodationQuote[] = ((payload.accommodationQuotes as any[]) || []).map((q: any) => ({
          ...
          board_type: q.board_type ?? null,
          board_notes: q.board_notes ?? null,
  ...
  const addItem = useCallback(async (
    ...
    const { data: blockRow, error } = await supabase
      ...
      .eq("id", blockId)
      .single();
    const block = blockRow as (typeof blockRow & { provider?: { id: string; name: string; email?: string } | null }) | null;
