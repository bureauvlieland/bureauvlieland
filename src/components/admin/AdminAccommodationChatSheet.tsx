...
export function AdminAccommodationChatSheet({
  open,
  onOpenChange,
  accommodationId,
  quoteId,
  ...
  partnerName,
  partnerEmail,
}: AdminAccommodationChatSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="flex flex-col w-full sm:max-w-md">
        <SheetHeader>
          <SheetTitle className="text-base">Berichten — {partnerName}</SheetTitle>
        </SheetHeader>
          ...
          <div className="space-y-3 py-4">
            ...
            {!isLoading && messages.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-8">
                Nog geen berichten. Stuur een bericht naar {partnerName}.
              </p>
            )}
                  ...
                  <div className={`flex ${isAdmin ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[80%] rounded-lg px-3 py-2 text-sm ${
                      ...
                      <p className={`text-[10px] mt-1 ${isAdmin ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
                        {format(msgDate, "HH:mm", { locale: nl })}
                      </p>
                    </div>
                  </div>
                </div>
