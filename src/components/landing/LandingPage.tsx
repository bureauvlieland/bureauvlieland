...
import type { LandingContent, LandingSection } from "@/content/landings/types";
...
const Paragraphs = ({ items, className }: { items: string[]; className?: string }) => (
...
const Closing = ({ text }: { text?: string }) =>
  text ? <p className="mt-8 text-lg font-medium leading-relaxed text-foreground">{renderRichText(text)}</p> : null;

const BodySection = ({ section, tone, eyebrow, number }: { section: LandingSection; tone: SectionTone; eyebrow: string; number: string }) => {
  switch (section.kind) {
    case "prose":
      return (
        <Section tone={tone}>
