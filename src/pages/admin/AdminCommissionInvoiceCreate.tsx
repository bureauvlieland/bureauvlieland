...
export default function AdminCommissionInvoiceCreate() {
  ...
  >([]);
  /** Logiesregels waarvoor met deze partner een afwijkend extra's-percentage geldt. */
  const [extrasToCheck, setExtrasToCheck] = useState<string[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setExtrasToCheck(drafts.filter((d) => d.extrasRateMismatch).map((d) => d.blockName));
    ...
    <>
      ...
      <AdminLayout>
        <div className="p-6 space-y-6">
          ...
          {extrasToCheck.length > 0 && (
            <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
              <p className="font-medium">Controleer het percentage over de extra&rsquo;s</p>
              <p className="mt-1">
                Met deze partner geldt voor extra&rsquo;s (F&amp;B, faciliteiten) een ander
                commissiepercentage dan voor de kamerprijs. Deze regels rekenen met één
                percentage over het geheel — pas de grondslag of het percentage zo nodig
                met de hand aan:
              </p>
              <ul className="mt-1 list-disc pl-5 space-y-0.5">
                {extrasToCheck.map((label) => (
