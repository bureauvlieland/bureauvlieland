...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: item, error: itemError } = await supabase
      ...
      .select(
        "*, program_requests!inner(id, reference_number, customer_name, customer_email, customer_company, customer_token, number_of_people, selected_dates)",
      )
      .eq("id", item_id)
      .single();
    ...
    const customerEmail = program.customer_email;
    if (!customerEmail) {
      return new Response(JSON.stringify({ error: "Geen klant-e-mailadres" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
        <h2 style="color: #1a365d; border-bottom: 2px solid #1a365d; padding-bottom: 10px;">
          Prijswijziging in uw programma
        </h2>
        <p>Geachte ${sanitizeHtml(program.customer_name)},</p>
        <p>De prijs van een onderdeel in uw programma is door Bureau Vlieland aangepast.
           Wij vragen u vriendelijk om de nieuwe prijs in uw klantportaal te bekijken en opnieuw uw akkoord te geven.</p>
        <div style="background: #fff7ed; padding: 20px; border-left: 4px solid #f59e0b; border-radius: 8px; margin: 20px 0;">
          ...
          <p style="margin: 0 0 8px;"><strong>Aantal personen:</strong> ${effectivePeople}</p>
          <p style="margin: 0 0 8px;"><strong>Nieuwe prijs:</strong> ${formatEur(newTotal)} ${isPerPerson ? `(${formatEur(Number(item.admin_price_override))} ${item.price_type === "per_person_per_day" ? "p.p.p.d." : "p.p."})` : "(totaal)"}</p>
        ...
        <p style="color: #666; font-size: 13px;">
          Heeft u vragen over deze wijziging? Reageer dan gerust op deze e-mail.
        </p>
    ...
    console.error("notify-customer-price-change error:", error);
