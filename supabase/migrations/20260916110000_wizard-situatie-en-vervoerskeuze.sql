-- Wizard fase 1: situatie van de groep, vervoerskeuze en tijdvak op de aanvraag.
-- Zie docs/plan-wizard-situatie-en-beschikbaarheid.md.
--
-- group_situation : 'vanaf_wal' | 'op_vlieland'
-- crossing_choice : 'doeksen' | 'watertaxi' | 'regina' | 'eigen' (null als al op Vlieland)
-- bike_choice     : 'standaard' | 'ebike' | 'eigen' | 'geen'
-- start_location  : accommodatie/adres waar het programma begint (al op Vlieland)
-- arrival_time    : begin van het tijdvak (aankomst, of start bij "al op Vlieland")
-- departure_time  : einde van het tijdvak (vertrek, of einde bij "al op Vlieland")

ALTER TABLE public.program_requests
  ADD COLUMN IF NOT EXISTS group_situation text
    CHECK (group_situation IS NULL OR group_situation IN ('vanaf_wal', 'op_vlieland')),
  ADD COLUMN IF NOT EXISTS crossing_choice text
    CHECK (crossing_choice IS NULL OR crossing_choice IN ('doeksen', 'watertaxi', 'regina', 'eigen')),
  ADD COLUMN IF NOT EXISTS bike_choice text
    CHECK (bike_choice IS NULL OR bike_choice IN ('standaard', 'ebike', 'eigen', 'geen')),
  ADD COLUMN IF NOT EXISTS start_location text,
  ADD COLUMN IF NOT EXISTS arrival_time time,
  ADD COLUMN IF NOT EXISTS departure_time time;

COMMENT ON COLUMN public.program_requests.group_situation IS 'Wizard: vanaf_wal of op_vlieland';
COMMENT ON COLUMN public.program_requests.crossing_choice IS 'Wizard: doeksen, watertaxi, regina of eigen';
COMMENT ON COLUMN public.program_requests.bike_choice IS 'Wizard: standaard, ebike, eigen (heeft al fietsen) of geen';
COMMENT ON COLUMN public.program_requests.start_location IS 'Wizard: startpunt op Vlieland (accommodatie of adres)';
COMMENT ON COLUMN public.program_requests.arrival_time IS 'Wizard: begin tijdvak op het eiland';
COMMENT ON COLUMN public.program_requests.departure_time IS 'Wizard: einde tijdvak op het eiland';

CREATE OR REPLACE FUNCTION public.submit_self_service_program_request(p_request jsonb, p_items jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  v_id uuid;
  v_ref text;
  v_token text;
  v_item jsonb;
  v_count int;
  v_origin text;
BEGIN
  IF p_items IS NULL OR jsonb_typeof(p_items) <> 'array' OR jsonb_array_length(p_items) = 0 THEN
    RAISE EXCEPTION 'submit geweigerd: minimaal 1 program_request_item vereist'
      USING ERRCODE = 'check_violation';
  END IF;

  v_id := COALESCE((p_request->>'id')::uuid, gen_random_uuid());
  v_token := p_request->>'customer_token';
  IF v_token IS NULL OR length(v_token) < 8 THEN
    RAISE EXCEPTION 'customer_token ontbreekt of ongeldig' USING ERRCODE = 'check_violation';
  END IF;

  v_origin := COALESCE(NULLIF(p_request->>'origin',''), 'self_service');

  INSERT INTO public.program_requests (
    id, customer_token, customer_name, customer_email, customer_phone, customer_company,
    number_of_people, selected_dates, general_notes, dietary_notes, origin, program_description,
    quote_status, attribution,
    catering_location_text, catering_start_time, has_horeca_on_site,
    group_situation, crossing_choice, bike_choice, start_location, arrival_time, departure_time
  ) VALUES (
    v_id,
    v_token,
    p_request->>'customer_name',
    p_request->>'customer_email',
    p_request->>'customer_phone',
    NULLIF(p_request->>'customer_company',''),
    (p_request->>'number_of_people')::int,
    -- selected_dates is jsonb in program_requests; valideer elke waarde als datum
    -- maar bewaar het als jsonb-array van yyyy-mm-dd strings.
    COALESCE((
      SELECT jsonb_agg(to_jsonb(to_char(d::date, 'YYYY-MM-DD')) ORDER BY d::date)
      FROM jsonb_array_elements_text(COALESCE(p_request->'selected_dates', '[]'::jsonb)) AS t(d)
    ), '[]'::jsonb),
    NULLIF(p_request->>'general_notes',''),
    NULLIF(p_request->>'dietary_notes',''),
    v_origin,
    COALESCE(NULLIF(p_request->>'program_description',''), 'niet_gespecificeerd'),
    'concept',
    COALESCE(p_request->'attribution', '{}'::jsonb),
    NULLIF(p_request->>'catering_location_text',''),
    NULLIF(p_request->>'catering_start_time','')::time,
    COALESCE((p_request->>'has_horeca_on_site')::boolean, false),
    NULLIF(p_request->>'group_situation',''),
    NULLIF(p_request->>'crossing_choice',''),
    NULLIF(p_request->>'bike_choice',''),
    NULLIF(left(p_request->>'start_location', 200),''),
    NULLIF(p_request->>'arrival_time','')::time,
    NULLIF(p_request->>'departure_time','')::time
  )
  RETURNING reference_number INTO v_ref;

  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    INSERT INTO public.program_request_items (
      request_id, block_id, block_name, block_category, provider_name, provider_id,
      provider_email, block_type, price_indication, day_index, preferred_time,
      customer_notes, status, skip_partner_notification, price_type, external_url,
      admin_price_override, location_lat, location_lng, location_address, admin_price_notes
    ) VALUES (
      v_id,
      v_item->>'block_id',
      v_item->>'block_name',
      v_item->>'block_category',
      v_item->>'provider_name',
      v_item->>'provider_id',
      v_item->>'provider_email',
      v_item->>'block_type',
      v_item->>'price_indication',
      COALESCE((v_item->>'day_index')::int, 0),
      v_item->>'preferred_time',
      v_item->>'customer_notes',
      COALESCE(v_item->>'status','pending'),
      COALESCE((v_item->>'skip_partner_notification')::boolean, true),
      COALESCE(v_item->>'price_type','per_person'),
      v_item->>'external_url',
      NULLIF(v_item->>'admin_price_override','')::numeric,
      NULLIF(v_item->>'location_lat','')::numeric,
      NULLIF(v_item->>'location_lng','')::numeric,
      v_item->>'location_address',
      v_item->>'admin_price_notes'
    );
  END LOOP;

  SELECT COUNT(*) INTO v_count FROM public.program_request_items WHERE request_id = v_id;
  IF v_count = 0 THEN
    RAISE EXCEPTION 'submit afgebroken: 0 items na insert' USING ERRCODE = 'check_violation';
  END IF;

  INSERT INTO public.program_request_history (request_id, action, actor, actor_name, new_value)
  VALUES (v_id, 'created', 'customer', p_request->>'customer_name',
          jsonb_build_object('items_count', v_count, 'origin', v_origin));

  RETURN jsonb_build_object('id', v_id, 'reference_number', v_ref, 'customer_token', v_token);
END;
$function$;

GRANT EXECUTE ON FUNCTION public.submit_self_service_program_request(jsonb, jsonb) TO anon, authenticated, service_role;
