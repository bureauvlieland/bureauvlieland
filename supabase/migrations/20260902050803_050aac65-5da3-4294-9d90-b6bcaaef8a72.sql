CREATE OR REPLACE FUNCTION public.get_scheduled_job_health()
 RETURNS TABLE(jobid bigint, jobname text, schedule text, active boolean, last_run_start timestamp with time zone, last_run_status text, last_http_status integer, last_http_error text, outcome text, runs_last_24h integer, failures_last_7d integer)
 LANGUAGE plpgsql
...
begin
  -- SECURITY DEFINER maakt current_user gelijk aan de eigenaar, dus die is
  -- géén bruikbaar signaal. We kijken naar de rol uit het verzoek (PostgREST)
  -- én naar session_user, zodat de cron-watchdog met de service-rol werkt.
  if not (
